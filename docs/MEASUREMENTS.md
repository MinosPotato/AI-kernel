...
| Authorization latency | `AuthorizationDecided::duration_ms`, `ToolInvoked::authorization_duration_ms` | **Locally exact** (wall-clock) |
| Approval latency | Same event, when `outcome` is `ApprovalGranted`/`ApprovalRefused`/`ApprovalUnavailable` | **Locally exact**, but *is* authorization latency for that phase — there is no separate "asked a human" sub-event, because the approval wait **is** what `decide()` is doing for that phase |
...
| G — approval | `--write` + a `require_approval` write policy, answered `y` | tool ran, file written; `AuthorizationDecided.duration_ms` for the `ApprovalGranted` decision was ~0ms in this pass because the scripted `y` answer arrived immediately — see [limitations](#limitations) |
