...
//!
//! Deliberately a copy of the other providers' module of the same name rather than a shared
//! one, for the reason `aik-anthropic` gives: sixty lines of timeout arithmetic are not worth
//! a crate that every provider must agree on, and how long a request may run is exactly the
